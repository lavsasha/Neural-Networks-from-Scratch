...
namespace NeuralNets {
    LearningRate::LearningRate(Scalar init_rate) : schedule_(LRSchedule::Constant), init_rate_(init_rate),
                                                       reduct_coef_(1) {}
    LearningRate::LearningRate(Scalar init_rate, LRSchedule schedule)
    ...
    LearningRate::LearningRate(Scalar init_rate, Scalar reduct_coef)
            : schedule_(LRSchedule::Exponential), init_rate_(init_rate), reduct_coef_(reduct_coef) {}

    Scalar LearningRate::GetRate(Index epoch) const {
        switch (schedule_) {
